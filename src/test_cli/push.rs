...
pub async fn push_tdf(addr: EndpointAddr, attribute_fqn: &str, data: &[u8]) -> Result<()> {
    let tdf_bytes = create_tdf_bytes(attribute_fqn, data).context("Failed to create TDF")?;
